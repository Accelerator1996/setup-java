...
export class DragonwellDistribution extends JavaBase {
  ...
  protected async findPackageForDownload(version: string): Promise<JavaDownloadRelease> {
    ...
    var edition = majorVersion == '17' ? 'Standard' : 'Extended';
    ...
    if (!resolvedVersion) {
      var versionsMsg = core.isDebug() ? ' Available versions: ${availableVersions}' : '';
  ...
  private async getAvailableVersions(): Promise<IDragonwellVersions[]> {
    ...
    var arch = this.distributionArchitecture();
    const releaseType = this.stable ? 'ga' : 'fp';

    var majorVersion = this.version.includes('.') ? this.version.split('.')[0] : this.version;
    if (['8', '11', '17'].includes(majorVersion) != true) {
      throw new Error('Support dragonwell versions: 8, 11, 17');
    }

    if (core.isDebug()) {
      console.time('dragonwell-retrieve-available-versions');
    }
    ...
    const fetchDragonwellVersions =
      (await this.http.getJson<IDragonwellAllVersions>(availableVersionsUrl)).result ?? {};
    ...
    if (core.isDebug()) {
      ...
      console.timeEnd('dragonwell-retrieve-available-versions');
      core.endGroup();
    }
  ...
  private getEligibleAvailableVersions(
    ...
    for (const majorVersion in dragonwellVersions) {
      ...
      for (var jdkVersion in majorVersionMap) {
        ...
        if (jdkVersion.includes('.')) {
          var splits = jdkVersion.split('.');
          jdkVersion = splits.length >= 3 ? splits.slice(0, 3).join('.') : jdkVersion;
        }
